...
## Estimated time
2h30 (includes the first end-to-end deploy).
...
## Tasks
1. `security.py`: `hash_password`/`verify_password` via `passlib` CryptContext(bcrypt). `create_access_token(sub, role)` → pyjwt HS256 with `exp` from `ACCESS_TOKEN_TTL_MIN`. `decode_access_token`. `new_refresh_token()` → returns `(raw, sha256_hex)`; only the hash is stored.
...
8. `auth_service.py`:
   ...
   - `refresh(raw)`: hash, look up. If not found → `AuthError`. **If found but `revoked_at` set → `revoke_all_for_user` + `AuthError` (reuse detection).** Else revoke this one, mint new pair, return.
...
11. `slowapi` on `POST /auth/login`: init a `Limiter(key_func=get_remote_address)`, register the exception handler, decorate login with `@limiter.limit("5/minute")`. Five lines; a visible brute-force defence.
...
- More than 5 logins/min from one IP → `slowapi` 429.
...
- `backend/tests/test_auth.py`: **refresh rotation + reuse detection.** Login → refresh rotates (old token now 401); replaying an already-revoked refresh token → 401 **and** revokes the whole family (a subsequent refresh with any of that user's tokens → 401). Also: register forces role `csm`; bad password → 401 generic. Monkeypatch nothing external; use `TestClient` with a fresh client to simulate the replayed/stolen token. This is the highest-value security test — write it here while the code is fresh.
...
1. **Skeleton backend live on Render**, with **Neon Postgres** and **Upstash Redis** connected; `alembic upgrade head` run against Neon; `/healthz` green on the Render URL.
