...
- **Custom domain (live):** frontend also serves on `https://csp.vishaljadeja.xyz` (GoDaddy CNAME `csp` → Vercel target; TLS cert issued, verified `HTTP 200`). No frontend/backend code or `BACKEND_URL` change was needed. Cookie has no `Domain` attribute, so it stays host-scoped and won't leak to the portfolio apex. Remember to add `https://csp.vishaljadeja.xyz` to Render's `CORS_ORIGINS` if not already done (cosmetic — proxy stays same-origin, not load-bearing).
